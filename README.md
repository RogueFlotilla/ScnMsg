...
This Python module is reponsible for downloading emails from the IMAP server set in the settings menu, saving each email to a pickel file, and creating a list of dictionaries that represent summary data for all emails. It has an additional function that can be called by other modules to load a single pickel file and return the needed contents from the object.
| Code Image *(click to enlarge)* | Description |
|-------------------------------|-------------|
...
| ![Screenshot of the function that calls the Groq API](./readme_images/analyze_groq.png)&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; | This portion of the code is the `check_email_for_spam` function. The function makes a Groq API call to use Llama 3 70B to analyze the contents of the email and assign a category. |
